...
function gigx_newspost ($news, $twocol_row = false, $breadcrumb = '', $show_img=true){
	...
	$postID = $post->ID;
	$img=wp_get_attachment_image_src (get_post_thumbnail_id($post->ID),'shows-image',false);
	...
	if($img && $show_img){
		...
		$img_html = '<div class="wp-caption alignleft" style="width: 310px"><a href="' . esc_url($permalink) . '" title="' . esc_attr($title) . '"><img src="'.$img[0].'" width="'.$img[1].'" height="'.$img[2].'" alt="'.$p->post_title.'" title="'.$post->post_title.'"/></a><p class="wp-caption-text">'.$title.'</p></div>';
	    }     
	    else $img_html = '<div class="wp-caption alignleft" style="width: 310px"><img src="'.$img[0].'" width="'.$img[1].'" height="'.$img[2].'" alt="'.$p->post_title.'" title="'.$post->post_title.'"/><p class="wp-caption-text">'.get_the_title().'</p></div>';
	}    
	...
	$out .= '	<span class="postdate">Posted by ' . get_the_author() . ' on '. get_the_time('jS F Y') .'</span>'; 
